...
def getHTMLFromPlotly(priceDF, tickerInfo, plotTimeframe = 90):
    ...
    ax.update_xaxes(range=[threeMonthsAgo, latest], row=1, col=1)
    ax.update_yaxes(range=[0,maxPrice+10], row=1, col=1)

    # mark the point where the prediction begins
                                # the first half of the graph originates in real values, the other in precited ones
    predictionMark = priceDF.index[plotTimeframe//2*-1]
